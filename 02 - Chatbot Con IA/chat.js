...
(function () {
  ...
  const BOTON = {
    contacto: function () { return { etiqueta: '📞 Contactar con Secretaría', alPulsar: mostrarContacto }; },
  ...
  function mostrarMenuPrincipal() {
    ...
    if (estado.config.funcionalidades.test_orientacion_4eso) {
    ...
    if (estado.config.funcionalidades.calendario_escolar) {
      opciones.push({ etiqueta: '📅 Calendario escolar y festivos', alPulsar: function () { pintarCalendario(true); } });
    }
  ...
  function mostrarPreguntasCategoria(idCategoria) {
    ...
    const opciones = (cat.preguntas || []).map(function (preg) {
      return {
        ...
        alPulsar: function () {
          mostrarRespuestaLocal({ pregunta: preg, categoria: cat.id, puntuacion: 99 });
        }
      };
    });
  ...
  function pintarCalendario(conBotones) {
    ...
    if (conBotones !== false) {
      mensajeBot('¿Necesitas algo más?', crearBotones([
  ...
  function arrancar(opciones) {
    ...
    Promise.all([
      cargarJSON(base + 'configuracion_centro.json').catch(function () { return null; }),
      ...
      if (resultados[1] && resultados[1].categorias) {
        estado.conocimiento = resultados[1];
      }
